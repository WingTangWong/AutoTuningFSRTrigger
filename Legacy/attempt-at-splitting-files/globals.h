  ...
  // Various global variables the system will use 
  // Define them and set initial values

  #include "boards.h"    // Board definitions and related variables

  bool DEBUG;

  // By default, will be Normally Closed
  // Note, if you want to daisy chain modules, they all need to be configured with the same setting
  bool normallyCLOSED = true;

// Values 
int ambient[MAX_SENSORS]={0,0,0};
int reading[MAX_SENSORS]={0,0,0};
int newambient[MAX_SENSORS]={0,0,0};
int delta=0;
int range=0;
int sensorValue=0;
int signalON=HIGH;
...
// ========================================================
// Global Variable Declarations for pins. (to be setup later in code)
// ========================================================
//
int outputPIN
int triggerPIN
int calibratePIN;
int analogPIN[MAX_SENSORS]; 
int ledPIN[MAX_SENSORS];
